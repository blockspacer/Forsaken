...
// Forsaken Addition: Location based damage modifiers
ConVar	sk_player_head( "sk_player_head","3" );
ConVar	sk_player_chest( "sk_player_chest","2.5" );
ConVar	sk_player_stomach( "sk_player_stomach","2" );
ConVar	sk_player_arm( "sk_player_arm","1.0" );
ConVar	sk_player_leg( "sk_player_leg","1.0" );
...
void CBasePlayer::SetAnimation( PLAYER_ANIM playerAnim )
{
	...
	// Forsaken Addition: Custom Player Anims
	else if (playerAnim == PLAYER_FSKN_SPRINT)
		idealActivity = ACT_FSKN_SPRINT;
	else if (playerAnim == PLAYER_FSKN_LEANLEFT)
		idealActivity = ACT_FSKN_LEANLEFT;
	else if (playerAnim == PLAYER_FSKN_LEANRIGHT)
		idealActivity = ACT_FSKN_LEANRIGHT;
...
bool CBasePlayer::StartObserverMode(int mode)
{
	if ( !IsObserver() )
	{
		...
		// Forsaken Addition: Reset player FOV.
		SetFOV(this, 0, 0.0f);
	}
	...
	// Forsaken Addition: If we are showing credits, don't show the spectator menu.
	if (Q_stricmp(STRING(gpGlobals->mapname), "credits"))
...
void CBasePlayer::ChangeTeam( int iTeamNum )
{
	...
	// Forsaken Addition
	// If we aren't in spectator, set us to living, we can take damage, and spawn us.
	if (iTeamNum != TEAM_SPECTATOR)
		ForsakenGameRules()->OnTeamChange(this);
}
...
// -------------------------------------------------------------------------------- //
	...
	BEGIN_SEND_TABLE_NOBASE(CPlayerState, DT_PlayerState)
		...
		SendPropQAngles (SENDINFO(v_angle), 24),
	END_SEND_TABLE()
...
// -------------------------------------------------------------------------------- //
	...
	IMPLEMENT_SERVERCLASS_ST( CBasePlayer, DT_BasePlayer )
		...
		SendPropInt		(SENDINFO(m_iMaxHealth), 10), // Forsaken Addition: Max Health
...
bool CBasePlayer::SetFOV( CBaseEntity *pRequester, int FOV, float zoomRate )
{
	...
	// Forsaken Addition: Add in a check to see if we aren't resetting the zoom.
	if ( ( m_hZoomOwner != NULL ) && ( m_hZoomOwner != pRequester ) && ( FOV != 0 ) )
	{
