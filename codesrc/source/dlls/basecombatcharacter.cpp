...
// Forsaken Addition: For some reason RemoveWeapon wasn't implemented. So I implemented it.
...
*/
{
	...
	for (int i = 0; i < MAX_WEAPONS; i++)
	{
		// If this is the weapon I'm looking for, delete it.
		if (m_hMyWeapons[i] == pWeapon)
		{
			// Delete the weapon and set it to NULL.
			m_hMyWeapons[i]->Delete();
			m_hMyWeapons.Set(i, NULL);
...
//-----------------------------------------------------------------------------
// Purpose: Set the player's ammo.
// Input  : iCount - Amount of ammo to set.
//			iAmmoIndex - Index of the ammo into the AmmoInfoArray
//			iMax - Max carrying capability of the player
// Output : Amount of ammo actually set
//-----------------------------------------------------------------------------
// Forsaken Addition: Set ammo amounts.
int CBaseCombatCharacter::SetAmmo( int iCount, int iAmmoIndex, bool bSuppressSound)
{
	if (iCount < 0)
		return 0;
...
int CBaseCombatCharacter::SetAmmo( int iCount, const char *szName, bool bSuppressSound )
{
	...
	return SetAmmo( iCount, iAmmoType, bSuppressSound );
}
