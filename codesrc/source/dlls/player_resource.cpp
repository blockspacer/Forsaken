...
// Datatable
// Forsaken Addition: Send kills and leader status.
...
//	SendPropArray( SendPropInt( SENDINFO_ARRAY(m_iPacketloss), 7, SPROP_UNSIGNED ), m_iPacketloss ),
	...
	SendPropArray3( SENDINFO_ARRAY3(m_bIsLeader), SendPropInt( SENDINFO_ARRAY(m_bIsLeader), 1, SPROP_UNSIGNED ) ),
END_SEND_TABLE()
...
void CPlayerResource::UpdatePlayerData( void )
{
	...
	{
		CForsakenPlayer *pPlayer = (CForsakenPlayer*)UTIL_PlayerByIndex( i );
		...
		if ( pPlayer && pPlayer->IsConnected() )
		{
			...
			// Forsaken Addition: Update kills and leader status.
			m_iKills.Set( i, pPlayer->FragCount() );
			m_bIsLeader.Set(i, pPlayer->IsLeader() );
