...
BEGIN_DATADESC( CBaseDoor )
	...
	// Forsaken Addition: Define the field
	DEFINE_FIELD(m_nTeamActivator, FIELD_INTEGER),
...
void CBaseDoor::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	// Forsaken Addition: Ignore multiple messages from the same team.
	if (m_nTeamActivator == pActivator->GetTeamNumber())
		return;
	else
		m_nTeamActivator = pActivator->GetTeamNumber();
...
void CBaseDoor::DoorHitTop( void )
{
	...
	if (HasSpawnFlags(SF_DOOR_START_OPEN) )
	{
		...
		// Forsaken Addition: Reset the team activator flags.
		m_nTeamActivator = TEAM_UNASSIGNED;
	}
	else
	{
