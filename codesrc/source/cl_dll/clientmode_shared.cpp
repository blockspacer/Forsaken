...
class CForsakenHudWeaponSelection;
class CForsakenHudChat;
...
void ClientModeShared::Init()
{
	if ( IsPC() )
	{
		m_pChatElement = ( CBaseHudChat * )GET_HUDELEMENT( CForsakenHudChat );
		Assert( m_pChatElement );
	}

	m_pWeaponSelection = ( CBaseHudWeaponSelection * )GET_HUDELEMENT( CForsakenHudWeaponSelection );
...
int	ClientModeShared::KeyInput( int down, int keynum, const char *pszCurrentBinding )
{
	...
	// Forsaken Addition: Don't display the spectator menu if we are in the credits.
	if( pPlayer && pPlayer->GetObserverMode() > OBS_MODE_DEATHCAM && Q_stricmp(engine->GetLevelName(), "credits") ) 
	{
...
{
	CBaseHudChat *hudChat = (CBaseHudChat *)GET_HUDELEMENT( CForsakenHudChat );
