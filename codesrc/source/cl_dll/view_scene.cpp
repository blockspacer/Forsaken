...
CLIENTEFFECT_REGISTER_BEGIN( PrecachePostProcessingEffects )
	...
	CLIENTEFFECT_MATERIAL( "effects/m84stun_flash" )
...
void CViewRender::Render2DEffectsPostHUD( const CViewSetup &view )
{
	...
	if (pPlayer->IsStunned() && g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80)
	{
		float fTime = (pPlayer->m_fStunEndTime - gpGlobals->curtime) / FORSAKEN_M84STUN_TIME;
		IMaterial *pMaterial = materials->FindMaterial("effects/m84stun_flash", 
			TEXTURE_GROUP_CLIENT_EFFECTS, true);
		IMaterialVar *pTime = NULL;

		// If we found the stun effect texture, do the effect.
		if (pMaterial)
		{
			bool bFound = false;

			// Download the contents of the frame buffer.
			UpdateScreenEffectTexture(0, view.x, view.y, view.width, view.height);

			// Find the time constant.
			pTime = pMaterial->FindVar("$timeConstant", &bFound, false);

			// Update the time constant.
			if (pTime && bFound)
				pTime->SetFloatValue(fTime);
			...
			// Draw the flash-bang effect.
			materials->DrawScreenSpaceQuad(pMaterial);

			// Roll back the changes to the projection and view matrix.
			materials->MatrixMode(MATERIAL_PROJECTION);
			materials->PopMatrix();
			materials->MatrixMode(MATERIAL_VIEW);
			materials->PopMatrix();
