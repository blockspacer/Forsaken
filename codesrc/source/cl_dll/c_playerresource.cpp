...
IMPLEMENT_CLIENTCLASS_DT_NOBASE(C_PlayerResource, DT_PlayerResource, CPlayerResource)
	...
	RecvPropArray3( RECVINFO_ARRAY(m_iKills), RecvPropInt( RECVINFO(m_iScore[0]))),
	...
	// Forsaken Addition: Receive leader flag.
	RecvPropArray3(RECVINFO_ARRAY(m_bIsLeader), RecvPropInt(RECVINFO(m_bIsLeader[0]))),
END_RECV_TABLE()
...
//	memset( m_iPacketloss, 0, sizeof( m_iPacketloss ) );
	...
	memset( m_iKills, 0, sizeof( m_iScore ) );
	...
	memset( m_bIsLeader, 0, sizeof( m_bIsLeader ) );
	...
	// Forsaken Addition
	// Purpose: set colors for the team scoreboard sections
	m_Colors[TEAM_UNASSIGNED] = COLOR_GREEN;
	m_Colors[TEAM_CIVILIANS] = COLOR_BLUE;
	m_Colors[TEAM_RELIGIOUS] = COLOR_RED;
	m_Colors[TEAM_GOVERNORS] = Color(155, 151, 151, 255);
	m_Colors[TEAM_SPECTATOR] = Color(203, 176, 154, 255);
...
// Purpose: 
//-----------------------------------------------------------------------------
// Forsaken Addition: Leader information
bool C_PlayerResource::IsLeader(int nIndex)
{
	return m_bIsLeader[nIndex];
}
