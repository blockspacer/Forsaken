...
{
	CForsakenWeaponBase *pWeapon = dynamic_cast<CForsakenWeaponBase *>(this);
	int nBrassAttachment = LookupAttachment("brass_eject");
	int nMuzzleAttachment = LookupAttachment("muzzle");

	if (pWeapon && (pWeapon->GetWeaponID() == FORSAKEN_WEAPON_DUALM10 || 
		pWeapon->GetWeaponID() == FORSAKEN_WEAPON_DUALP99))
	{
		CForsakenPlayer *pPlayer = pWeapon->GetPlayerOwner();

		if (pPlayer && (pPlayer->m_nShotsFired & 0x1))
			nMuzzleAttachment = LookupAttachment("muzzle_l");
		else if (pPlayer)
			nMuzzleAttachment = LookupAttachment("muzzle_r");
	}
	...
	if ( muzzleflash_light.GetBool() )
	{
		...
		if ( m_Attachments.Count() > 0 )
		{
			Vector vAttachment;
			QAngle dummyAngles;
			GetAttachment( nMuzzleAttachment, vAttachment, dummyAngles );

			// Make an dlight
			dlight_t *pMuzzleFlash = effects->CL_AllocDlight( LIGHT_INDEX_MUZZLEFLASH + index );

			pMuzzleFlash->origin = vAttachment;
			pMuzzleFlash->radius = 70;
			pMuzzleFlash->decay = pMuzzleFlash->radius / 0.05f;
			pMuzzleFlash->die = gpGlobals->curtime + 0.05f;
			pMuzzleFlash->color.r = 255;
			pMuzzleFlash->color.g = 192;
			pMuzzleFlash->color.b = 64;
			pMuzzleFlash->color.exponent = 5;
		}
	}

	// Do we have a valid wepaon?
	if (pWeapon)
	{
		// Do we have a muzzle attachment?
		if (nMuzzleAttachment >= 0)
			tempents->MuzzleFlash(pWeapon->GetMuzzleFlashType(), GetRefEHandle(), nMuzzleAttachment, false);

		// If we have a valid player and brass ejection attachment, do a world shell ejection.
		if (nBrassAttachment >= 0)
		{
			CForsakenPlayer *pPlayer = dynamic_cast<C_ForsakenPlayer*>(pWeapon->GetOwner());
			QAngle angBrassEject;
			Vector vecBrassEjectPos;

			if (pPlayer)
			{
				// Obtain the position and angle for brass ejection.
				GetAttachment(nBrassAttachment, vecBrassEjectPos, angBrassEject);

				// Fire the eject brass event.
				tempents->CSEjectBrass(vecBrassEjectPos, angBrassEject, 100, 
					pWeapon->GetShellType(), pPlayer);
			}
		}
	}
}
...
bool C_BaseAnimating::DispatchMuzzleEffect( const char *options, bool isFirstPerson )
{
	...
	{
		// Forsaken Addition
		// Here we add in our own parsing for muzzle flashes.
		if (Q_stricmp(token, "1201FP") == 0)
			weaponType = FSKN_MUZZLEFLASH_1201FP;
		else if (Q_stricmp(token, "710") == 0)
			weaponType = FSKN_MUZZLEFLASH_710;
		else if (Q_stricmp(token, "DualM10") == 0)
			weaponType = FSKN_MUZZLEFLASH_DUALM10;
		else if (Q_stricmp(token, "DualP99") == 0)
			weaponType = FSKN_MUZZLEFLASH_DUALP99;
		else if (Q_stricmp(token, "HK23") == 0)
			weaponType = FSKN_MUZZLEFLASH_HK23;
		else if (Q_stricmp(token, "RagingBull") == 0)
			weaponType = FSKN_MUZZLEFLASH_RAGINGBULL;
		else if (Q_stricmp(token, "SA80") == 0)
			weaponType = FSKN_MUZZLEFLASH_SA80;
		else
			Assert(0);
		/*
...
bool C_BaseAnimating::TestHitboxes( const Ray_t &ray, unsigned int fContentsMask, trace_t& tr )
{
	...
	// Forsaken Addition: We need to prevent the deletion of models.
	MDLCACHE_CRITICAL_SECTION();
