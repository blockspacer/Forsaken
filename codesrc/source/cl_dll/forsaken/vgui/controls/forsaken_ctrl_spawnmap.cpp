...
// HL2 Class Macros
//DECLARE_HUDELEMENT(CForsakenSpawnMap);

// Constructor & Deconstructor
CForsakenSpawnMap::CForsakenSpawnMap(vgui::Panel *pParent, const char *czName, 
...
CForsakenSpawnMap::~CForsakenSpawnMap()
{
}
void CForsakenSpawnMap::Update()
/*
...
void CForsakenSpawnMap::UpdateObjectiveButtons()
/*
...
*/
{
	...
	for(int butIndex = 0; butIndex < m_vecObjectiveButtons.Count(); butIndex++)
	{
		...
		{
			// Who owns the spawn?
			switch (m_vecObjectives[butIndex].nTeamBitmask)
			{
			case TEAM_CIVILIANS:
			...
			// If we own it we can spawn there.
			enabled = (pPlayer->GetTeamNumber() == m_vecObjectives[butIndex].nTeamBitmask);
		}
		else if (m_vecObjectives[butIndex].nLocationType == LOCATION_TYPE_OBJECTIVE)
		{
			...
			if (pFortification)
			{
					...
					pFortification->GetFortificationStatus() == FORSAKEN_FORTIFIEDSTATUS_UPGRADING)
				{
					...
					// If we own it and it's fortified we can spawn there.
					enabled = (pPlayer->GetTeamNumber() == pObjective->GetTeamOwner());
				}
			}
		...
		msg->SetInt("selectionType", SELECT_TYPE_SPAWN);
