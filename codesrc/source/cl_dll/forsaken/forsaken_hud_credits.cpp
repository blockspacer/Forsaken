...
// Forsaken Addition: Play credits command.
CON_COMMAND(play_credits, "Plays the credits for Forsaken.")
{
	// Set the server to a LAN server and then go to the credits map.
	engine->ExecuteClientCmd("sv_lan 1");
	engine->ExecuteClientCmd("map credits");
}


CON_COMMAND ( creditsdone, "Notifies the game that the credits are done." )
{
	if (!Q_stricmp(engine->GetLevelName(), "credits"))
	{
		engine->ExecuteClientCmd("sv_lan 0");
