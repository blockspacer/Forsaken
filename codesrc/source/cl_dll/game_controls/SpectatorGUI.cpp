...
//	m_iDuckKey = KEY_NONE;
	...
	// Forsaken Addition: Create the labels for the resource totals.
	m_pCivNutritionResources = new Label(this, "civnutrition", "");
	m_pCivOrdinanceResources = new Label(this, "civordinance", "");
	m_pGovNutritionResources = new Label(this, "govnutrition", "");
	m_pGovOrdinanceResources = new Label(this, "govordinance", "");
	m_pRelNutritionResources = new Label(this, "relnutrition", "");
	m_pRelOrdinanceResources = new Label(this, "relordinance", "");
...
void CSpectatorGUI::Update()
{
	...
	// Forsaken Addition: Update resource totals.
	{
		if (g_Teams.Count() == 0)
			return;
		...
		char czNutrition[256] = "", czOrdinance[256] = "";

		// Update the civilians resource totals.
		if (pCivTeam)
		{
			Q_snprintf(czNutrition, sizeof(czNutrition), "%d", pCivTeam->GetNutritionResources());
			Q_snprintf(czOrdinance, sizeof(czOrdinance), "%d", pCivTeam->GetOrdinanceResources());

			m_pCivNutritionResources->SetText(czNutrition);
			m_pCivOrdinanceResources->SetText(czOrdinance);
		}

		// Update the governors resource totals.
		if (pGovTeam)
		{
			Q_snprintf(czNutrition, sizeof(czNutrition), "%d", pGovTeam->GetNutritionResources());
			Q_snprintf(czOrdinance, sizeof(czOrdinance), "%d", pGovTeam->GetOrdinanceResources());

			m_pGovNutritionResources->SetText(czNutrition);
			m_pGovOrdinanceResources->SetText(czOrdinance);
		}

		// Update the religious resource totals.
		if (pRelTeam)
		{
			Q_snprintf(czNutrition, sizeof(czNutrition), "%d", pRelTeam->GetNutritionResources());
			Q_snprintf(czOrdinance, sizeof(czOrdinance), "%d", pRelTeam->GetOrdinanceResources());

			m_pRelNutritionResources->SetText(czNutrition);
			m_pRelOrdinanceResources->SetText(czOrdinance);
		}
	}
}
...
void CSpectatorGUI::UpdateTimer()
{
	...
	// Forsaken Addition: Fetch the round time.
	int timer = ForsakenGameRules()->GetRemainingTimeInRound();
