...
#include "forsaken/vgui/forsaken_roundwin.h"
#include "forsaken/vgui/forsaken_mapwin.h"
...
#include "forsaken/vgui/forsaken_leaderoptions.h"
...
CON_COMMAND ( mapoverview, "Opens a dialog showing map goals and war status" )
{
	if (!gViewPortInterface)
		return;

	gViewPortInterface->ShowPanel(PANEL_MAPOVERVIEW, true);
}
...
CON_COMMAND ( leader_options, "Opens a dialog showing leader options" )
{
	if (!gViewPortInterface)
		return;

	// BETA DISABLE: Disable the leader options panel for release.
#ifdef BETA_FORSAKEN
	gViewPortInterface->ShowPanel(PANEL_LEADEROPTIONS, true);
#endif
}
...
void CBaseViewport::CreateDefaultPanels( void )
{
	...
	AddNewPanel( CreatePanelByName( PANEL_ROUNDWIN ) );
	AddNewPanel( CreatePanelByName( PANEL_MAPWIN ) );
	AddNewPanel( CreatePanelByName( PANEL_MAPOVERVIEW ) );
	AddNewPanel( CreatePanelByName( PANEL_INGAMEHELP ) );
	AddNewPanel( CreatePanelByName( PANEL_WARSUMMARY ) );
	AddNewPanel( CreatePanelByName( PANEL_LEADEROPTIONS ) );
#endif
}
...
/*	else if ( Q_strcmp(PANEL_OVERVIEW, szPanelName) == 0 )
	...
	*/
	...
	else if ( Q_strcmp(PANEL_ROUNDWIN, szPanelName) == 0 )
	{
		newpanel = new CRoundWinMessage( this );
	}
	else if ( Q_strcmp(PANEL_MAPWIN, szPanelName) == 0 )
	{
		newpanel = new CMapWinMessage( this );
	}
	else if ( Q_strcmp(PANEL_MAPOVERVIEW, szPanelName) == 0 )
	{
		newpanel = new CForsakenMapOverview( this );
	}
	...
	else if ( Q_strcmp(PANEL_LEADEROPTIONS, szPanelName) == 0 )
	{
		newpanel = new CLeaderOptions( this );
	}
#endif
