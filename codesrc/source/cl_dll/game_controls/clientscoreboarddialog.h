...
class CClientScoreBoardDialog : public vgui::Frame, public IViewPortPanel, public IGameEventListener2
{
	...
	// column widths at 640
		enum { NAME_WIDTH = 280, SCORE_WIDTH = 35, KILL_WIDTH = 35, DEATH_WIDTH = 40, PING_WIDTH = 40 };
		// total = 340
	...
	protected:
		...
		virtual bool GetPlayerData(int playerIndex, KeyValues *outPlayerInfo);
		virtual void InitScoreboardSections();
		virtual void UpdateTeamInfo();
		virtual void UpdatePlayerInfo();
		
		virtual void AddHeader();		// scoreboard headers
		virtual int AddSection(int teamType, int teamNumber); // new team section
		...
		int	m_iSectionId;			// global current editing section
	...
	private:
		...
		int iTeamSections[5];		// sectionID references for the window sections
		int numPlayersOnTeam[5];	// # of players on team
		int teamPing[5];			// array of team pings
