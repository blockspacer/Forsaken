...
// Forsaken Addition: We need to include these header files.
#include "forsaken/forsaken_weapon_base.h"
#include "forsaken/forsaken_weapon_base_gun.h"
#include "forsaken/forsaken_weapon_base_rifle.h"
...
//-----------------------------------------------------------------------------
// Forsaken Addition: Bug Fix ID 0000068, we need to handle events we don't draw.
/*
	Bug Fix ID 0000068: We need to be able to play weapon sounds, but we don't want to be seen.
	In order to accomplish this we need to return ShouldDraw to false if we aren't doing an 
	animation and we are zoomed in.
*/
bool C_BaseViewModel::ShouldDraw()
{
	// Fetch a pointer to our weapon.
	CForsakenWeaponRifleBase *pRifleParent = dynamic_cast<CForsakenWeaponRifleBase*>(GetWeapon());

	// If the weapon is a rifle, we are zoomed in, and we aren't doing a zoom anim; don't draw.
	if (pRifleParent != NULL && pRifleParent->GetZoomLevel() != 0 && 
		pRifleParent->GetZoomAnimTimeEnd() <= gpGlobals->curtime)
		return false;
