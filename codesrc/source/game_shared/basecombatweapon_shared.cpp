...
{ 
	...
	else
	{
		...
		// Forsaken: Deploy after the holster animation is done.
		g_EventQueue.AddEvent(pSwitchingTo, "DeployWeapon", flSequenceDuration, NULL, NULL);
	}
#endif
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::InputDeployWeapon(inputdata_t &rInput)
{
	if (GetOwner())
	{
		Deploy();
