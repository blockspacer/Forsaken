...
bool CMultiplayRules::IsMultiplayer( void )
{
	...
	void CMultiplayRules::Think ( void )
	{
		...
		// Forsaken Addition: The game must be over and intermission must have ended.
	...
	void CMultiplayRules::PlayerKilled( CBasePlayer *pVictim, const CTakeDamageInfo &info )
	{
		...
		if ( pVictim == pScorer )  
		{
			...
			// Forsaken Addition: No losing kills for self-death.
	...
	void CMultiplayRules::DeathNotice( CBasePlayer *pVictim, const CTakeDamageInfo &info )
	{
		...
		else
		{
			...
			// Forsaken Addition: Strip out forsaken-specific weapon prefix's.
			else if (strncmp(killer_weapon_name, "forsaken_", 9) == 0)
			{
		...
		if ( event )
		{
			...
			// Forsaken Addition: Broadcast weapon name.
