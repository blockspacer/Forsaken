...
void PropertySheet::PerformLayout()
{
	...
	if (_activePage)
	{
		...
		if(_showTabs)
		{
			// Forsaken Addition: We want to go by the bounds of the tab sheet.
			_activePage->SetBounds(x, y + tabHeight, wide, tall - tabHeight);
		}
		else
		{
			// Forsaken Addition: We want to go by the bounds of the tab sheet.
			_activePage->SetBounds(x, y, wide, tall );
		}
	...
	if (_showTabs)
	{
		for (int i = 0; i < limit; i++)
		{
			...
			if (m_PageTabs[i] == _activeTab)
			{
				...
				_activeTab->SetBounds(x + xtab, y + 2, width, tabHeight);
			}
			else
			{
				// Forsaken Addition: We want to go by the bounds of the tab sheet.
				m_PageTabs[i]->SetBounds(x + xtab, y + 4, width, tabHeight - 2);
			}
...
void PropertySheet::OnKeyCodeTyped(KeyCode code)
{
	...
	if ( IsKBNavigationEnabled() )
	{
		...
		case KEY_RIGHT:
			{
				// Forsaken Addition: We only want to go to the next tab that is available.
				newindex = _activeTabIndex + 1;

				// Go until we loop back around to where we are.
				while (newindex != _activeTabIndex)
				{
					// We hit the end, so cycle back around.
					if (!m_PageTabs.IsValidIndex(newindex) && m_PageTabs.Count() > 0)
					{
						newindex = 0;
					...
					// Is this tab enabled? If so, go to it.
					if (m_PageTabs[newindex]->IsEnabled())
					{
		...
		case KEY_LEFT:
			{
				...
				while (newindex != _activeTabIndex)
				{
					// We hit the beginning, so cycle back around.
					if (!m_PageTabs.IsValidIndex(newindex) && m_PageTabs.Count() > 0)
					{
						newindex = m_PageTabs.Count() - 1;
